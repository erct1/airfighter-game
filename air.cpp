...
public:
    ...
    void setSkin(int t) {
        ...
        // ★ 根据皮肤设定最大武器等级
        if (t == SKIN_DEFAULT) {
            maxWeaponLevel = 3;   // 默认皮肤最高3级
        }
        else {
            maxWeaponLevel = 5;   // 其他皮肤可升到5级终极
        }
...
public:
    ...
    // ★ 重写 playerShoot 以支持 Lv.4 和 Lv.5
...
#define IS_CONFIRM (g_keys[VK_SPACE]||g_keys[VK_RETURN])
    ...
    const TCHAR* getWeaponLevelName(const Player& p) {
        ...
        if (p.weaponLevel <= 3) {
            ...
            _stprintf_s(buf, _T("★ 强化 Lv.4"));
        }
        else {
            const TCHAR* ultimateNames[SKIN_COUNT] = {
                _T("能量弹幕"), _T("烈焰风暴"), _T("冰晶散射"),
                _T("雷霆万钧"), _T("黄金爆裂"), _T("暗影突袭")
            };
            _stprintf_s(buf, _T("★★ %s"), ultimateNames[p.skinType]);
        }
        return buf;
    }
    ...
    void drawUI() {
        ...
        _stprintf_s(t, _T("武器: %s"), getWeaponLevelName(player1));
        outtextxy(20, 60, t);
        if (mode == MODE_COOP || (mode == MODE_SURVIVAL && survivalPlayerCount == 2)) {
            _stprintf_s(t, _T("武器: %s"), getWeaponLevelName(player2));
...
int main() {
    ...
    SetWindowText(GetHWnd(), _T("太空射击游戏 V1.6 - 终极武器"));
